...
if( !function_exists( 'wpmn_get_template' ) ) :
    /**
    ...
    function wpmn_get_template( $template_name, $args = array(), $template_path = '' ) {
        if( empty( $template_path ) ) :
            $template_path = WPMN_PATH . 'templates/';
        endif;        
        ...
        if ( ! file_exists( $template ) ) :
            return new WP_Error( 
                ...
                sprintf( 
                    ...
                    esc_html__( '%s does not exist.', 'medianest' ), 
                    '<code>' . esc_html( $template ). '</code>' 
                ) 
            );
        endif;

        do_action( 'wpmn_before_template_part', $template, $args, $template_path );
        ...
        do_action( 'wpmn_after_template_part', $template, $args, $template_path );
    }
