<?php

// Exit if accessed directly.
if ( ! defined( 'ABSPATH' ) ) exit;

if ( ! class_exists( 'MDDR_Media_Library' ) ) :

    /**
     * Main MDDR_Media_Library Class
     *
     * @class MDDR_Media_Library
     * @version 1.0.0
     */
	class MDDR_Media_Library {

        /**
         * Settings
         */
        public $settings;

		/**
		 * Constructor for the class.
		 */
		public function __construct() {
			$this->events_handler();
		}

        /**
         * Initialize hooks and filters.
         */
		public function events_handler() {
            $this->settings = get_option( 'mddr_settings', [] );
			add_action( 'admin_footer', array( $this, 'render_sidebar' ) );
            add_action( 'attachment_fields_to_edit', array( $this, 'add_folder_field' ), 10, 2 );
            add_filter( 'ajax_query_attachments_args', array( $this, 'filter_attachments' ) );
            add_action( 'pre_get_posts', array( $this, 'filter_list_view' ) );
            add_action( 'pre_get_posts', array( $this, 'filesize_orderby' ) );
            add_filter( 'manage_media_columns', array( $this, 'add_folder_column' ) );
            add_filter( 'manage_media_columns', array( $this, 'add_file_size_column' ) );
            add_action( 'manage_media_custom_column', array( $this, 'display_file_size_column' ), 10, 2 );
            add_action( 'manage_media_custom_column', array( $this, 'display_folder_column' ), 10, 2 );
            add_filter( 'manage_upload_sortable_columns', array( $this, 'register_file_size_sortable' ) );
            add_action( 'add_attachment', array( $this, 'save_attachment_size' ) );
            add_action( 'edit_attachment', array( $this, 'save_attachment_size' ) );

            
            // Add columns for other supported post types
            $post_types = isset( $this->settings['post_types'] ) ? (array) $this->settings['post_types'] : [];
            
            foreach ( $post_types as $pt ) :
            add_filter( "manage_edit-{$pt}_columns", array( $this, 'add_folder_column' ) );
                add_filter( "manage_{$pt}_posts_columns", array( $this, 'add_folder_column' ) );
                add_action( "manage_{$pt}_posts_custom_column", array( $this, 'display_folder_column' ), 10, 2 );
            endforeach;

            do_action( 'mddr_media_library_init', $this );
            add_action( 'elementor/editor/footer', array( $this, 'render_sidebar' ) );
            add_action( 'wp_footer', function () {
                if ( function_exists( 'et_core_is_fb_enabled' ) && et_core_is_fb_enabled() ) {
                    $this->render_sidebar();
                }
            });

		}

        public function render_sidebar() {
            $screen = get_current_screen();
            if ( ! $screen ) return;

            $enabled_post_types = isset( $this->settings['post_types'] ) ? (array) $this->settings['post_types'] : [];

            $media               = ( $screen->id === 'upload' );
            $supported_post_type = in_array( $screen->post_type, $enabled_post_types );
            $list_view           = ( $screen->base === 'edit' );
            $post_edit           = ( $screen->base === 'post' );

            if ( $media || $post_edit || ( $supported_post_type && $list_view ) ) :
                $this->output_sidebar();
            endif;
        }

		public function output_sidebar() {
            $mddr_labels = MDDR_Helper::get_folder_labels();
            
			mddr_get_template(
				'media/library-sidebar.php',
				array(
                    'mddr_labels' => $mddr_labels,
                ),
			);
		}

        public function filter_attachments( $query ) {

            if ( empty( $_REQUEST['query']['mddr_folder'] ) ) :
                return $query;
            endif;

            // Proper way to handle this without phpcs:ignore is to verify the core nonce
            check_ajax_referer( 'query-attachments', 'nonce', false );

            $folder = sanitize_text_field( wp_unslash( $_REQUEST['query']['mddr_folder'] ) );
            if ( $folder === 'uncategorized' ) :

                $args = array(
                    'taxonomy'   => 'mddr_media_folder',
                    'fields'     => 'ids',
                    'hide_empty' => false,
                );

                // Check for User Specific Folder Mode logic to match sidebar behavior
                $user_mode = isset($this->settings['user_separate_folders']) && $this->settings['user_separate_folders'] === 'yes';

                if ( $user_mode && is_user_logged_in() ) :
                    $args['meta_query'] = array(
                        array(
                            'key'     => 'mddr_folder_author',
                            'value'   => get_current_user_id(),
                            'compare' => '=',
                        )
                    );
                endif;

                $terms = get_terms( $args );

                if ( ! empty( $terms ) ) :
                    $query['tax_query'] = array(
                        array(
                            'taxonomy' => 'mddr_media_folder',
                            'field'    => 'term_id',
                            'terms'    => $terms,
                            'operator' => 'NOT IN',
                        )
                    );
                endif;
                return $query;
            endif;

            if ( strpos( $folder, 'term-' ) === 0 ) :
                $term_id = absint( str_replace( 'term-', '', $folder ) );
                if ( $term_id ) :
                    $query['tax_query'] = array(
                        array(
                            'taxonomy'         => 'mddr_media_folder',
                            'field'            => 'term_id',
                            'terms'            => $term_id,
                            'include_children' => false,
                        )
                    );
                endif;
            endif;

            // Handle sorting by file size
            if ( isset( $_REQUEST['query']['orderby'] ) && 'mddr_filesize' === $_REQUEST['query']['orderby'] ) :
                check_ajax_referer( 'query-attachments', 'nonce', false );
                $query['meta_key'] = 'mddr_filesize';
                $query['orderby']  = 'meta_value_num';
            endif;

            return $query;
        }

        public function filter_list_view( $query ) {
            if ( ! is_admin() || ! $query->is_main_query() ) return;

            $folder = ! empty( $_GET['mddr_folder'] ) ? sanitize_text_field( wp_unslash( $_GET['mddr_folder'] ) ) : '';
            if ( ! $folder || $folder === 'all' ) return;

            $screen    = get_current_screen();
            $post_type = '';

            if ( $screen ) :
                $post_type = $screen->post_type;
            elseif ( isset( $_GET['post_type'] ) ) :
                $post_type = sanitize_text_field( wp_unslash( $_GET['post_type'] ) );
            else :
                $post_type = 'post';
            endif;

            // Always allow attachment, otherwise check settings
            if ( $post_type !== 'attachment' ) :
                $enabled_post_types = isset( $this->settings['post_types'] ) ? (array) $this->settings['post_types'] : [];

                if ( ! in_array( $post_type, $enabled_post_types ) ) return;
            endif;

            if ( $folder === 'uncategorized' ) :
                $args = array(
                    'taxonomy'   => 'mddr_media_folder',
                    'fields'     => 'ids',
                    'hide_empty' => false,
                    'meta_query' => array(
                        array(
                            'key'     => 'mddr_post_type',
                            'value'   => $post_type,
                            'compare' => '=',
                        ),
                    ),
                );

                $user_mode = isset($this->settings['user_separate_folders']) && $this->settings['user_separate_folders'] === 'yes';

                if ( $user_mode && is_user_logged_in() ) :
                    $args['meta_query']['relation'] = 'AND';
                    $args['meta_query'][] = array(
                        'key'     => 'mddr_folder_author',
                        'value'   => get_current_user_id(),
                        'compare' => '=',
                    );
                endif;

                $terms = get_terms( $args );

                if ( ! empty( $terms ) ) :
                    $query->set( 'tax_query', array(
                        array(
                            'taxonomy' => 'mddr_media_folder',
                            'field'    => 'term_id',
                            'terms'    => $terms,
                            'operator' => 'NOT IN',
                        )
                    ) );
                endif;
                return;
            endif;

            if ( 0 === strpos( $folder, 'term-' ) ) :
                $term_id = absint( str_replace( 'term-', '', $folder ) );
                if ( $term_id ) :
                    $query->set( 'tax_query', array(
                        array(
                            'taxonomy'         => 'mddr_media_folder',
                            'field'            => 'term_id',
                            'terms'            => $term_id,
                            'include_children' => false,
                        )
                    ) );
                endif;
            endif;
        }

        public function filesize_orderby( $query ) {
            if ( ! is_admin() || ! $query->is_main_query() ) return;

            if ( 'mddr_filesize' === $query->get( 'orderby' ) ) :
                $query->set( 'meta_key', 'mddr_filesize' );
                $query->set( 'orderby', 'meta_value_num' );
            endif;
        }

        public function add_folder_field( $form_fields, $post ) {
            $terms       = wp_get_object_terms( $post->ID, 'mddr_media_folder' );
            $labels      = MDDR_Helper::get_folder_labels();
            $post_type   = get_post_type( $post->ID );
            
            $meta_query = array(
                'relation' => 'OR',
                array(
                    'key'     => 'mddr_post_type',
                    'value'   => $post_type,
                    'compare' => '=',
                ),
            );

            if ( $post_type === 'attachment' ) :
                $meta_query[] = array(
                    'key'     => 'mddr_post_type',
                    'compare' => 'NOT EXISTS',
                );
            endif;

            // Check if user_separate_folders setting is enabled
            $user_mode = isset($this->settings['user_separate_folders']) && $this->settings['user_separate_folders'] === 'yes';

            if ( $user_mode && is_user_logged_in() ) :
                $current_user_id = get_current_user_id();
                $author_query = array(
                    'relation' => 'OR',
                    array(
                        'key'     => 'mddr_folder_author',
                        'value'   => $current_user_id,
                        'compare' => '=',
                    ),
                    array(
                        'key'     => 'mddr_folder_author',
                        'compare' => 'NOT EXISTS',
                    )
                );

                $meta_query = array(
                    'relation' => 'AND',
                    $meta_query,
                    $author_query
                );
            endif;

            $include_terms = get_terms( array(
                'taxonomy'   => 'mddr_media_folder',
                'hide_empty' => false,
                'fields'     => 'ids',
                'meta_query' => $meta_query,
            ) );

            $select_html = wp_dropdown_categories(array(
                'taxonomy'          => 'mddr_media_folder',
                'hide_empty'        => false,
                'name'              => 'mddr_media_folder_select',
                'id'                => 'mddr_media_folder_select_' . $post->ID,
                'class'             => 'mddr_folder_dropdown',
                'show_option_all'   => $labels['all'],
                'show_option_none'  => $labels['uncategorized'],
                'option_none_value' => '0',
                'selected'          => ( isset( $terms[0] ) && isset( $terms[0]->term_id ) ) ? $terms[0]->term_id : 0,
                'include'           => ! empty( $include_terms ) ? $include_terms : array( -1 ),
                'echo'              => 0,
                'hierarchical'      => true,
            ) );
            $select_html = str_replace('&nbsp;&nbsp;&nbsp;', '- ', $select_html);

            $form_fields['mddr_media_folder'] = array(
                'label' => esc_html__( 'Media Directory Folder', 'media-directory' ),
                'helps' => esc_html__( 'Click the folder name to move this file to a different folder', 'media-directory' ),
                'input' => 'html',
                'html'  => $select_html . '<span class="spinner mddr_folder_loader"></span>',
            );

            return $form_fields;
        }

        public function add_folder_column( $columns ) {
            $columns['mddr_folder_col'] = esc_html__( 'Media Directory Folder', 'media-directory' );
            return $columns;
        }

        public function display_folder_column( $column_name, $id ) {

            if ( 'mddr_folder_col' !== $column_name ) return;

            $terms          = get_the_terms( $id, 'mddr_media_folder' );
            $post_type      = get_post_type( $id );
            $is_attachment  = ( 'attachment' === $post_type );
            $base_url       = $is_attachment ? 'upload.php' : 'edit.php';
            $query_args     = array();

            if ( $is_attachment ) :
                $query_args['mode'] = 'list';
            else :
                $query_args['post_type'] = $post_type;
            endif;

            if ( ! empty( $terms ) && ! is_wp_error( $terms ) ) :
                $folder_links = array();
                foreach ( $terms as $term ) :
                    $query_args['mddr_folder'] = 'term-' . $term->term_id;
                    $url = add_query_arg( $query_args, admin_url( $base_url ) );
                    
                    $folder_links[] = sprintf(
                        '<a href="%s">%s</a>',
                        esc_url( $url ),
                        esc_html( $term->name )
                    );
                endforeach;
                echo wp_kses_post( implode( ', ', $folder_links ) );
            else :
                $query_args['mddr_folder'] = 'uncategorized';
                $url = add_query_arg( $query_args, admin_url( $base_url ) );
                
                echo sprintf(
                    '<a href="%s">%s</a>',
                    esc_url( $url ),
                    esc_html__( 'Uncategorized', 'media-directory' )
                );
            endif;
        }

        public function add_file_size_column( $columns ) {
            $columns['mddr_filesize'] = esc_html__( 'File Size', 'media-directory' );
            return $columns;
        }

        public function display_file_size_column( $column_name, $id ) {
            if ( 'mddr_filesize' !== $column_name ) return;

            $bytes = get_post_meta( $id, 'mddr_filesize', true );

            if ( ! $bytes ) :
                $file_path = get_attached_file( $id );
                require_once ABSPATH . 'wp-admin/includes/file.php';
                if ( ! function_exists( 'WP_Filesystem' ) ) {
                    require_once ABSPATH . 'wp-admin/includes/file.php';
                }
                WP_Filesystem();
                global $wp_filesystem;

                if ( $file_path && $wp_filesystem->exists( $file_path ) ) :
                    $bytes = $wp_filesystem->size( $file_path );
                    update_post_meta( $id, 'mddr_filesize', $bytes );
                endif;
            endif;
            
            if ( $bytes ) :
                echo esc_html( size_format( $bytes, 2 ) );
            else :
                echo '—';
            endif;
        }

        public function save_attachment_size( $post_id ) {

            $file_path = get_attached_file( $post_id );
            require_once ABSPATH . 'wp-admin/includes/file.php';

            if ( ! function_exists( 'WP_Filesystem' ) ) :
                require_once ABSPATH . 'wp-admin/includes/file.php';
            endif;
            
            WP_Filesystem();
            global $wp_filesystem;

            if ( $file_path && $wp_filesystem->exists( $file_path ) ) :
                $bytes = $wp_filesystem->size( $file_path );
                update_post_meta( $post_id, 'mddr_filesize', $bytes );
            endif;
        }

        public function register_file_size_sortable( $columns ) {
            $columns['mddr_filesize'] = 'mddr_filesize';
            return $columns;
        }
	}

	new MDDR_Media_Library();

endif;
