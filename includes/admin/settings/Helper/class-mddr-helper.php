...
if ( ! class_exists( 'MDDR_Helper' ) ) :

	/**
     * Main MDDR_Helper Class
     *
     * @class MDDR_Helper
     * @version 1.0.0
     */
	class MDDR_Helper {
		...
		public static function get_folder_labels() {
			return array(
				'choose_folder'  => esc_html__( 'Choose folder:', 'media-directory' ),
		...
		public static function create_folder( $name, $parent ) {
			...
			$result = wp_insert_term( $name, 'mddr_media_folder', $args );
			...
			if ( is_wp_error( $result ) && $result->get_error_code() === 'term_exists' ) :
				...
				while ( $counter <= 100 ) :
                    ...
                    if ( $user_mode && is_user_logged_in() ) :
                         ...
                         $dup_result = wp_insert_term( $new_name, 'mddr_media_folder', $dup_args );
                         
						if ( ! is_wp_error( $dup_result ) ) :
							return $dup_result;
						endif;
                    else :
                        if ( ! term_exists( $new_name, 'mddr_media_folder', $parent ) ) :
		...
		public static function create_folder_request() {
            ...
            if ( ! is_wp_error( $result ) && isset( $result['term_id'] ) ) :
                update_term_meta( $result['term_id'], 'mddr_post_type', $post_type );
                update_term_meta( $result['term_id'], 'mddr_folder_author', get_current_user_id() );
            endif;
		...
		public static function rename_folder_request() {
			...
			$result = wp_update_term($id, 'mddr_media_folder', array(
				'name' => $name,
		...
		public static function delete_folder_recursively($id) {
			...
			$children = get_term_children($id, 'mddr_media_folder');
			if (!is_wp_error($children) && !empty($children)) :
				foreach ($children as $child_id) :
					wp_delete_term($child_id, 'mddr_media_folder');
				endforeach;
			endif;

			return wp_delete_term($id, 'mddr_media_folder');
		}
		...
		public static function assign_media_request() {
			...
			$mddr_terms = ( $folder_id > 0 ) ? array( $folder_id ) : array();

			foreach ($items as $attachment_id) :
				wp_set_object_terms( $attachment_id, $mddr_terms, 'mddr_media_folder', false );
		...
		public static function clear_all_data_request() {
			...
			if ( ! is_wp_error( $terms ) ) :
				foreach ($terms as $term) :
					wp_delete_term($term->term_id, 'mddr_media_folder');
				endforeach;
			endif;

			delete_option('mddr_settings');
			wp_send_json_success(['message' => esc_html__('All data cleared.', 'media-directory')]);
		}
		...
		public static function move_folder_request() {
			...
			if (is_wp_error(get_term($folder_id, 'mddr_media_folder'))) :
				wp_send_json_error(['message' => 'Folder not found.']);
			endif;

			$result = wp_update_term($folder_id, 'mddr_media_folder', array(
				'parent' => $new_parent
			) );
			self::send_response($result, $post_type);
		}
        ...
        public static function generate_attachment_size_request() {
            ...
            foreach ( $attachments as $id ) :	
                ...
                if ( $path && file_exists( $path ) ) :
                    $size = filesize( $path );
                    update_post_meta( $id, 'mddr_filesize', $size );
                    $count++;
                endif;
            endforeach;
            ...
            wp_send_json_success( array(
                ...
                'message' => sprintf( esc_html__( 'Generated sizes for %d attachments.', 'media-directory' ), $count )
            ) );
        }
        ...
        public static function save_settings_request() {
            ...
            if ( ! $user_id ) :
                wp_send_json_error( array( 'message' => esc_html__( 'User not logged in.', 'media-directory' ) ) );
            endif;
            ...
            if ( isset( $_POST['default_folder'] ) ) :
                update_user_meta( $user_id, 'mddr_default_folder', sanitize_text_field( wp_unslash( $_POST['default_folder'] ) ) );
            endif;

            if ( isset( $_POST['default_sort'] ) ) :
                update_user_meta( $user_id, 'mddr_default_sort', sanitize_text_field( wp_unslash( $_POST['default_sort'] ) ) );
            endif;

            if ( isset( $_POST['theme_design'] ) ) :
                update_user_meta( $user_id, 'mddr_theme_design', sanitize_text_field( wp_unslash( $_POST['theme_design'] ) ) );
            endif;

            wp_send_json_success( array(
                'message' => esc_html__( 'Settings saved successfully.', 'media-directory' )
            ) );
        }
	}
