...
if ( ! class_exists( 'MDDR_Reorder' ) ) :

	/**
     * Main MDDR_Reorder Class
     *
     * @class MDDR_Reorder
     * @version 1.0.0
     */
	class MDDR_Reorder {
		...
		public static function reorder_folder_request() {
			...
			$target_term = get_term($target_id, 'mddr_media_folder');
			if (!$target_term || is_wp_error($target_term)) :
				wp_send_json_error(['message' => 'Target folder not found.']);
			endif;

			wp_update_term($folder_id, 'mddr_media_folder', array(
			...
			foreach ($new_order as $index => $term_id) :
				update_term_meta($term_id, 'mddr_order', $index);
			endforeach;
