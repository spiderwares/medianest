...
if ( ! class_exists( 'MDDR_Import' ) ) :
	
	/**
     * Main MDDR_Import Class
     *
     * @class MDDR_Import
     * @version 1.0.0
     */
	class MDDR_Import {
		...
		public static function import_folders_request() {
			...
			foreach ($rows as $row) :
				...
				$result = wp_insert_term($name, 'mddr_media_folder', array(
				...
				if (!empty($row['created_by'])) :
					update_term_meta($new_id, 'mddr_folder_owner', absint($row['created_by']));
				endif;

				if (!empty($row['post_type'])) :
					update_term_meta($new_id, 'mddr_post_type', sanitize_text_field($row['post_type']));
				endif;
			endforeach;
			...
			foreach ($rows as $row) :
				...
				if (!empty($row['parent']) && !empty($id_map[$row['parent']])) :
					wp_update_term($new_id, 'mddr_media_folder', array(
				...
				if (!empty($row['attachment_ids'])) :
					...
					foreach ($att_ids as $att_id) :
						wp_set_object_terms($att_id, [$new_id], 'mddr_media_folder', false);
					endforeach;
				endif;
			endforeach;

			wp_send_json_success(MDDR_Media_Folders::payload());
		}
	}
