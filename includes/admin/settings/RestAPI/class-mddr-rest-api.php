...
if ( ! class_exists( 'MDDR_REST_API' ) ) :

    /**
     * Main MDDR_REST_API Class
     *
     * @class MDDR_REST_API
     * @version 1.0.0
     */
    class MDDR_REST_API {
        ...
        public function events_handler() {
            ...
            add_action( 'rest_api_init', [ $this, 'mddr_register_routes' ] );
        }

        /**
         * Register REST API routes
         */
        public function mddr_register_routes() {

            // GET https://your-site.com/wp-json/media-directory/v1/folders
            register_rest_route( 
                MDDR_REST_API_URL, '/folders', 
                array(
                'methods'             => 'GET',
            ...
            // GET https://your-site.com/wp-json/media-directory/v1/folder/?folder_id
            register_rest_route( 
                MDDR_REST_API_URL, '/folder', 
                array(
                'methods'             => 'GET',
            ...
            // POST https://your-site.com/wp-json/media-directory/v1/folder/set-attachment
            register_rest_route( 
                MDDR_REST_API_URL, '/folder/set-attachment', 
                array(
                'methods'             => 'POST',
            ...
            // GET https://your-site.com/wp-json/media-directory/v1/attachment-id/?folder_id=
            register_rest_route( 
                MDDR_REST_API_URL, '/attachment-id', 
                array(
                'methods'             => 'GET',
            ...
            // GET https://your-site.com/wp-json/media-directory/v1/attachment-count/?folder_id=
            register_rest_route( 
                MDDR_REST_API_URL, '/attachment-count', 
                array(
                'methods'             => 'GET',
            ...
            // POST https://your-site.com/wp-json/media-directory/v1/folders
            register_rest_route( 
                MDDR_REST_API_URL, '/folders', 
                array(
                'methods'             => 'POST',
        ...
        public static function generate_api_key_request() {
            ...
            $options = get_option( 'mddr_settings', [] );
            $options['rest_api_key'] = $key;
            update_option( 'mddr_settings', $options );

            wp_send_json_success( array(
                'key'     => $key,
                'message' => esc_html__( 'API Key generated successfully.', 'media-directory' )
            ));
        }
        ...
        public function is_valid_api_key( $request ) {
            ...
            $header_key = $request->get_header( 'Media-Directory-API-Key' );
        ...
        public function get_folders( $request ) {
            ...
            // Sort terms by mddr_order meta then term_id
            ...
            if ( ! empty( $search ) ) :
                ...
                foreach ( $terms as $index => $term ) :
                    $count = MDDR_Media_Folders::folder_count( $term->term_id );
                    $folders[] = array(
                        ...
                        'ord'        => (int) get_term_meta( $term->term_id, 'mddr_order', true ) ?: $index,
            ...
            foreach ($terms as $term) :
                $term->count_with_children = MDDR_Media_Folders::folder_count($term->term_id);
        ...
        public function get_folder_details($request) {
            ...
            $term = get_term($id, 'mddr_media_folder');
        ...
        public function set_attachment($request) {
            ...
            foreach ($ids as $att_id) :
                ...
                wp_set_object_terms($att_id, $folder_id, 'mddr_media_folder', false);
            ...
            wp_update_term_count_now([$folder_id], 'mddr_media_folder');
        ...
        public function get_attachment_count($request) {
            ...
            return rest_ensure_response(array(
                'success' => true,
                'data'    => array(
                    'count' => (string) MDDR_Media_Folders::folder_count($id)
                )
            ) );
        }
        ...
        public function create_new_folder($request) {
            ...
            $term = MDDR_Helper::create_folder($name, $parent_id);
            if (is_wp_error($term)) return $term;

            if ( isset( $term['term_id'] ) ) :
                update_term_meta( $term['term_id'], 'mddr_post_type', 'attachment' );
                
                if ( $user_id > 0 ) :
                    update_term_meta( $term['term_id'], 'mddr_folder_author', $user_id );
                endif;
            endif;
